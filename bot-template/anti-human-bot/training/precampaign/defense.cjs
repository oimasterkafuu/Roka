...
function chooseDefense(state) {
  ...
  if (!crowns.length || !enemies.length) { plans.delete(state); return null; }
  ...
  if (receipt && Number.isInteger(receipt.turn) && receipt.turn <= turn && receipt.turn > memory.receiptTurn) {
    ...
    if (pending && receipt.op === 'm' && receipt.turn > pending.turn && receipt.turn <= pending.turn + 1 &&
        ...
        receipt.half !== true && (receipt.mode == null || receipt.mode === 0)) {
  ...
  // 对每个兵团分别搜索，不能将全图敌兵直接加到皇冠旁。每层保留抵达兵力
  // 最大的简单路径；盟军格可汇合，本方/队友/中立守军均消耗入侵兵力。
  ...
  for (const c of crowns) {
    ...
    for (const e of enemies) {
      ...
      let best = null;
      for (let t = 1; t <= HORIZON && layer.size; t++) {
        const next = new Map();
        for (const [i, p] of layer) for (const j of ns(i)) {
          if (--budget < 0) return null;
          if (distance[j] < 0 || distance[j] > HORIZON - t || p.path.includes(j) || unknown(j) || grid[j] === 201) continue;
