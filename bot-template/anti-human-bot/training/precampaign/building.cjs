...
function crownTarget(land, turn, p) {
  // 中后期规模已够却仅有一个皇冠时，至少争取第二个生存锚点。
  return Math.min(Math.floor(p.maxCrowns), Math.max(
    1 + Math.floor(land / p.territoryPerCrown), land >= 20 && turn >= 100 ? 2 : 1));
}
function chooseBuild(state, move, params = {}) {
  ...
  const target = crownTarget(land, state.turn || 0, p);
  const unfinished = [];
...
module.exports = { chooseBuild, urgent, crownTarget };
