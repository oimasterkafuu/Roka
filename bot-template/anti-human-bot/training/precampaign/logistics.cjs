...
const { chooseBuild, urgent, crownTarget } = require('./building.cjs');
...
function chooseLogistics(state, move, build, params = {}) {
  ...
  if (urgent(state, move)) return null;
  ...
  const targetCrowns = crownTarget(lands.length, turn, p);
  ...
  function transport(target, funding = false) {
    ...
    for (const from of lands) {
      ...
      if (!funding && memory.economy && (memory.economy.target === from || memory.economy.funded.has(from)) && safe(from) &&
