...
exports.chooseMove = function chooseMove(state, params = {}) {
  ...
  for (const s of sources) {
    for (const t of neighbors[s]) {
      if (params.blockedEdges?.has(`${s}:${t}`)) continue;
      ...
      if (enemy(t) && (!params.allowedOwners || params.allowedOwners.has(owners[t])) && !unknown(t) && amount > count(t) + growthBy(t, 1) && crowns.some((c) => danger.has(c) && neighbors[c].includes(t)))
        offer(s, t, 110000 + amount - count(t), '清除主城威胁');
      if (enemy(t) && (!params.allowedOwners || params.allowedOwners.has(owners[t])) && grid[t] >= 101 && grid[t] <= 149 && !unknown(t) && amount > count(t) + growthBy(t, 1))
  ...
  for (const source of sources.slice(0, 24)) {
    ...
    for (let head = 0; head < paths.length && budget > 0; head++) {
      ...
      for (const t of neighbors[p.at]) {
        if (p.depth === 0 && params.blockedEdges?.has(`${source}:${t}`)) continue;
