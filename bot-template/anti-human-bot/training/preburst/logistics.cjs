...
function chooseLogistics(state, move, build, params = {}) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    if (neighbors[i].some(enemy)) fronts.push(i);
    if (neighbors[i].some(j => !allied(owners[j]) && grid[j] !== 201 && grid[j] !== 203)) borders.push(i);
  }
  ...
  const economicValid = plan => plan && turn < plan.until && safe(plan.target) && crowns < targetCrowns &&
  ...
  if (!memory.economy && crowns < targetCrowns) {
    const candidates = lands.filter(i => safe(i) &&
  ...
  if (economy && economicSlot && turn > memory.lastBuild) {
    const i = economy.target, upgrade = grid[i] === me + 50, reserve = reserveAt(i);
    if (count(i) >= 50 + reserve && (upgrade || budget(i) >= 100)) {
      memory.lastBuild = turn; memory.lastEconomy = turn;
      return { kind: 'build', x: Math.floor(i / m), y: i % m, op: upgrade ? 'c' : 'b',
        reason: { code: 'logistics-invest', phase: upgrade ? 'upgrade' : 'foundation', target: i, reserve } };
    }
  }
  let plan = memory.rally, phase = 'reinforce';
  // 筹资每四tick最多取得一次额外行动，不能长期吞掉真实前线运输。
  if (economy && economicSlot) {
    const goal = 50 + reserveAt(economy.target);
