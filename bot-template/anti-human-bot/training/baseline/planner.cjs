...
exports.chooseMove = function chooseMove(state) {
  ...
  for (const s of sources) {
    for (const t of neighbors[s]) {
      ...
      if (enemy(t) && !unknown(t) && amount > count(t) && crowns.some((c) => danger.has(c) && neighbors[c].includes(t)))
        offer(s, t, 110000 + amount - count(t), '清除主城威胁');
      if (enemy(t) && grid[t] >= 101 && grid[t] <= 149 && !unknown(t) && amount > count(t))
  ...
  for (const source of sources.slice(0, 24)) {
    ...
    for (let head = 0; head < paths.length && budget > 0; head++) {
      ...
      for (const t of neighbors[p.at]) {
        ...
        const defense = unknown(t) ? Math.max(2, count(t)) : count(t);
        ...
        if (danger.has(t)) { score = 100000; reason = '守主城'; }
          ...
          else { score = enemy(t) ? 1800 : 1500; reason = '拓地'; }
        } else if (!ours) { score = 700; reason = '探路'; }
        ...
        if (score > -Infinity && (!inward || strategic || distance[first] < distance[source]))
          offer(source, first, score - depth * 80 + Math.min(remaining, 30), reason);
