(function () {
  ...
  const STEP_LIST = [
    ...
    {
      title: '操作队列',
      body: '快速连按两次 D（或 →）：推兵指令会进入操作队列，每回合自动执行一条，格子上的小箭头标示排队方向。实战中可按 R 清空队列、按 F 撤销队尾一条。',
      hint: '连按排队，占领高亮格。',
    ...
    {
      title: 'X / Q 建造指挥所',
      body: '在己方普通地块（兵力 ≥ 50）按 X 或 Q，消耗 50 兵建造指挥所，下一回合建成。建造与推兵共用操作队列：入队只校验地形，执行时才校验归属与兵力，条件不满足会自动跳过；排队时格子右下角会显示小角标。指挥所每 50 Tick +1 兵，更是关键的连通锚点。',
      hint: '把 50 兵调到高亮空地（或任意己方空地），按 X 或 Q 建造。',
      targets: [POS.buildPlain],
    },
    {
      title: 'C / E 升级主城',
      body: '选中己方指挥所（兵力 ≥ 50）按 C 或 E，再消耗 50 兵升级为主城——每多一座主城，每 Tick 多产 1 兵。进阶技巧：在普通空地直接按 C / E 会自动连排「建指挥所 + 升级主城」两步，两回合建成，两个角标并排显示（共需 100 兵）。',
      hint: '给高亮指挥所补足 50 兵后，选中它按 C 或 E 升级。',
      targets: [],
      dynamic: 'playerCity',
    },
    {
  ...
  function enqueueBuild(op) {
    ...
    if (op === 'b') {
      ...
      // C/E：己方指挥所直接升级主城；其余可建地块连续入队「建 + 升」两步
      if (tile.terrain === 'crown') {
        return;
      }
