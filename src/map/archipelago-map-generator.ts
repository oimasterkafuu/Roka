...
const placeRectIsland = (island: IslandInfo, gridType: Tile[][], islandIdGrid: number[][]): void => {
...
const canPlaceMountainSafely = (n: number, m: number, gridType: Tile[][], x: number, y: number): boolean => {
  ...
  for (let d = 0; d < 4; d += 1) {
    ...
    if (!isInBounds(n, m, cx, cy) || (gridType[cx][cy] !== -1 && gridType[cx][cy] !== -2)) {
      continue;
    }
...
const placePlayerIslands = (
  ...
  for (let i = 0; i < selectedPlayerIslands.length; i += 1) {
    ...
    // Roka: no pre-placed city on player islands; island centers become mountains.
    for (let j = 0; j < island.centerCells.length; j += 1) {
      const [cx, cy] = island.centerCells[j];
      if (gridType[cx][cy] === 0 && canPlaceMountainSafely(n, m, gridType, cx, cy)) {
        gridType[cx][cy] = 1;
      }
    }
...
const placeNeutralIslands = (
  ...
  for (let i = 0; i < islands.length; i += 1) {
    ...
    // Roka: neutral islands hold mountains instead of pre-placed cities.
    ...
    for (let j = 0; j < centerCells.length; j += 1) {
      ...
      if (canPlaceMountainSafely(n, m, gridType, x, y)) {
        gridType[x][y] = 1;
      }
    }
  }
...
const generateArchipelagoMap = (rng: SeededRandom, config: ArchipelagoMapGenerationConfig): GeneratedMap => {
  ...
  for (let mapAttempt = 0; mapAttempt < MAP_RETRY_LIMIT; mapAttempt += 1) {
    ...
    if (!placePlayerIslands(rng, n, m, gridType, selectedPlayerIslands)) {
      continue;
    }

    placeNeutralIslands(rng, n, m, gridType, islands, playerIslandIds);
